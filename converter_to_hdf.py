...
with h5py.File(output_h5, 'w') as f:
    ...
    for t_idx, df_dens in tqdm(enumerate(pd.read_csv(files['data'], sep=r'\s+', header=None, names=column_names, 
                      dtype={'time_index': int}, chunksize=chunksize)), desc="Time step"):        
        step_data = df_dens['d_dens'].values
        #print(step_data)
